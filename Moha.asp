    ...
    <div class="header">
      <div class="footer">
        <div class="body">
          ...
          <div class="content" >
		  <div class="album" >
		  <div class="album_pho" style ="background:url(Moha.jpg);" >
		  </div>
		  <form action="moha.asp" method="post">
		  <h1>MOHAMED ALHERE </h1>
		  <fieldset>
          <label for="bio">Write a comment to him : </label>
		...
		qry="insert into comment (username,comment,comment_date,user_id)values('"&Session("username")&"','"&com&"','"&now&"',2)"
		oConn.execute(qry)
		oConn.close
		end if
		%>
		...
		Set oConn = Server.CreateObject("ADODB.Connection")
		oConn.Open("DRIVER={Microsoft Access Driver (*.mdb)}; Dbq=" & Server.MapPath("db.mdb"))
		qry="select * from comment where user_id=2"
